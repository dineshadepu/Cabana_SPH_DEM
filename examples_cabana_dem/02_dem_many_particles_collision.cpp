				      ...
				      const int write_freq)
{
  ...
  */
  ...
  std::vector<double> x = {
    0.,
    1. * (rigid_body_diameter + rigid_body_diameter / 20),
    2. * (rigid_body_diameter + rigid_body_diameter / 20),
    3. * (rigid_body_diameter + rigid_body_diameter / 20),
    4. * (rigid_body_diameter + rigid_body_diameter / 20),
  };

  std::vector<double> y = {
    0.,
    -rigid_body_diameter / 20,
    rigid_body_diameter / 20,
    -rigid_body_diameter / 20,
    rigid_body_diameter / 20,
  };
...
int main( int argc, char* argv[] )
{
  ...
  if ( argc < 7 )
	...
	<< "      write_freq      number of steps between output files\n";
      ...
      std::cerr << "\nfor example: ./01DEM 0.1 1.0 1.0 6 0.00013 10\n";
      Kokkos::finalize();
      MPI_Finalize();
      return 0;
    }
