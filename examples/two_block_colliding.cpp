...
void fluid_stage_3(AoSoAType & aosoa, double dt, int * limits){
  ...
  auto fluid_stage_3_lambda_func = KOKKOS_LAMBDA( const int i )
    {
      aosoa_velocity(i, 0) += aosoa_acc( i, 0 ) * half_dt;
      aosoa_velocity(i, 1) += aosoa_acc( i, 1 ) * half_dt;
      aosoa_velocity(i, 2) += aosoa_acc( i, 2 ) * half_dt;
...
void state_equation(AoSoAType & aosoa, ViewVectorType & b_rho0_p0, double dt, int * limits){
  ...
  auto state_equation_lambda_func = KOKKOS_LAMBDA( const int i )
    {
      double tmp = b_rho0_p0[2] * (aosoa_density( i ) / b_rho0_p0[1] - b_rho0_p0[0]);
      aosoa_p( i ) = tmp;
    };
  Kokkos::RangePolicy<ExecutionSpace> policy( limits[0], limits[1] );
  Kokkos::parallel_for( "CabanaSPH:Equations:state_equations", policy,
			state_equation_lambda_func );
}
...
void solid_wall_pressure_bc(AoSoAType & aosoa, ViewVectorType & gravity, double dt,
  ...
  auto make_p_zero_of_boundary_lambda_func = KOKKOS_LAMBDA( const int i )
    {
      aosoa_p( i ) = 0.;
      aosoa_sum_wij( i ) = 0.;
    };
  Kokkos::parallel_for( "CabanaSPH:Equations:make_p_zero_bdry", policy,
			make_p_zero_of_boundary_lambda_func );
  ...
  Kokkos::parallel_for( "CabanaSPH:Equations:divide_p_by_wij", policy,
			divide_p_by_wij_lambda_func );
}
...
void clamp_wall_pressure(AoSoAType & aosoa, double dt, int * limits){
  ...
  Kokkos::RangePolicy<ExecutionSpace> policy( limits[0], limits[1] );
  Kokkos::parallel_for( "CabanaSPH:Equations:clamp_p", policy,
			clamp_p_lambda_func );
}
...
void body_force(AoSoAType & aosoa, ViewVectorType & gravity, double dt,
  ...
  Kokkos::RangePolicy<ExecutionSpace> policy( limits[0], limits[1] );
  Kokkos::parallel_for( "CabanaSPH:Equations:body_force", policy,
...
//---------------------------------------------------------------------------//
void twoBlocksColliding(const double fluid_spacing,
			...
			const int write_freq)
{
  ...
  */
  ...
  int no_fluid_particles = 2 * x_fluid.size();
  int no_bdry_particles = 0;

  int total_no_particles = no_fluid_particles;
  ...
  for ( std::size_t i = 0; i < x_fluid.size(); ++i )
    {
      x.push_back(x_fluid[i] + fluid_length * 1.2);
      y.push_back(y_fluid[i]);
      z.push_back(z_fluid[i]);
      u.push_back(-1);
      is_fluid.push_back(1);
...
int main( int argc, char* argv[] )
{
  ...
  // run the problem.
  twoBlocksColliding( fluid_spacing, fluid_height, fluid_length,
