...
void compute_force_on_dem_particles(RBAoSoA rb,
				    ListType * verlet_list, int *limits,
				    double dt, double fric_coeff){
  auto rb_position = Cabana::slice<0>     ( rb,    "rb_position");
  auto rb_limits = Cabana::slice<1>       ( rb,      "rb_limits");
  auto rb_ids = Cabana::slice<2>          ( rb,         "rb_ids");
  auto rb_velocity = Cabana::slice<3>     ( rb,    "rb_velocity");
...
//---------------------------------------------------------------------------//
void rb_freely_rotating_square_bodies(const double fluid_spacing,
				      ...
				      const int write_freq)
{
  ...
  */
  ...
  auto rigid_body_radius = fluid_spacing / 2. - fluid_spacing / 20;
  ...
  for ( std::size_t i = 0; i < x_fluid.size(); ++i )
    {
      ...
      x_fluid[i] += (3) * fluid_spacing;
      y_fluid[i] += (2) * fluid_spacing;
    }
  x_fluid[0] += fluid_spacing / 30;
  y_fluid[0] += fluid_spacing / 30;
  // for ( std::size_t i = 0; i < x_fluid.size(); ++i )
  //   {
  ...
  for ( int step = 0; step < steps; step++ )
    {
      ...
      body_force_rigid_body(rb, fluid_limits, gravity, dt);

      update_tangential_contacts(rb, dt, fluid_limits);
      compute_force_on_dem_particles(rb,
				     &verlet_list, fluid_limits, dt, 0.1);
...
int main( int argc, char* argv[] )
{
  ...
  if ( argc < 8 )
	...
	<< "      write_freq      number of steps between output files\n";
      ...
      std::cerr << "\nfor example: ./03DEMDB 0.1 1.0 1.0 1.4 2.0 0.4 100\n";
      Kokkos::finalize();
      MPI_Finalize();
      return 0;
    }
  ...
  // run the problem.
  rb_freely_rotating_square_bodies( fluid_spacing, fluid_height, fluid_length,
...
//     contact_count.append(f["rb_contact_count"][2])

//     cnt = f["rb_contact_count"][2]
//     if cnt > 0:
//         # print(f["rb_contact_idx"][1])
//         print("----------------------------")
//         # print(f["rb_contact_tng_frc"][0])
//         # print(f["rb_contact_tng_frc"][1])
//         print(f["rb_contact_idx"][2])
//         print(f["rb_contact_tng_frc"][2])
//         print("----------------------------")
