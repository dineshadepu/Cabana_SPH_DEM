  ...
  auto dem_contact_force_kernel = KOKKOS_LAMBDA( const int i, const int j )
	...
	if (overlap > 0){
	  ...
	  */
	  ...
	  // double cor = 0.2;
	  // double tmp_01 = log(cor);
	  // double tmp_02 = pow(tmp_01, 2.) + pow(M_PI, 2.) ;
	  // double alpha_1 = -tmp_01 * sqrt(5. / tmp_02);
	  // double tmp_1 = 1. / mass_i;
	  // double tmp_2 = 1. / mass_j;
	  // double m_eff = 1. / (tmp_1 + tmp_2);
	  // double eta = alpha_1 * sqrt(m_eff * kn) * pow(overlap, 0.25);
				      ...
				      const int write_freq)
{
  ...
  /*
    ...
    ========================================================================
  */
  // dimentions of the system
  auto tank_length = 2. * 15 * body_length;
  auto tank_height = 1.3 * (no_of_bodies / 20.) * body_length;
  auto tank_layers = 3;
  ...
  */
  ...
  // Create the rigid body particles
  for ( std::size_t i = 0; i < no_of_bodies; ++i ) {
	...
	int fac = i % 20 ;
	int fac_1 = int (i / 20);
	x.push_back(x_rb[j] + fac * body_length + fac * 3. * body_spacing + 2. * body_spacing);

	y.push_back(y_rb[j] + fac_1 * body_length + fac_1 * 2 * body_spacing + body_height);
	// y.push_back(y_rb[j] + (i * 4. * body_spacing) + 15 * body_spacing);
  ...
  std::tie(x_tank, y_tank, z_tank) = create_2d_tank(tank_length, tank_height,
						    body_spacing, tank_layers);

  for ( std::size_t j = 0; j < x_tank.size(); ++j )
    {
      x.push_back(x_tank[j]);
      y.push_back(y_tank[j]);
      z.push_back(z_tank[j]);
  ...
  */
  ...
  std::cout << "Total no of particles are :" << num_particles << std::endl;
  /*
    ================================================
    End: Step 3
    ================================================
  */
  ...
  auto x_min_tank = *std::min_element(x_tank.begin(), x_tank.end());
  auto x_max_tank = *std::max_element(x_tank.begin(), x_tank.end());
  auto y_min_tank = *std::min_element(y_tank.begin(), y_tank.end());
  auto y_max_tank = *std::max_element(y_tank.begin(), y_tank.end());
  double grid_min[3] = { -5. * neighborhood_radius, -body_length,  -1. * neighborhood_radius };
  double grid_max[3] = { 1.3 * tank_length, 1.3 * tank_height, 1. * neighborhood_radius };
...
int main( int argc, char* argv[] )
{
  ...
  if ( argc < 7 )
	...
	"body_length no_of_bodies t_final write_freq\n";
