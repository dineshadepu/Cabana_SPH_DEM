...
void set_body_limits_of_rb(AoSoAType aosoa, RBAoSoA rb){
  auto aosoa_body_id = Cabana::slice<17>     ( aosoa,    "body_id");
  auto rb_limits = Cabana::slice<1>       ( rb,      "rb_limits");
...
void set_linear_velocity_rigid_body(AoSoAType aosoa, RBAoSoA rb, int body_no, std::vector<double>& lin_vel){
...
void set_angular_velocity_rigid_body(AoSoAType aosoa, RBAoSoA rb, int body_no, std::vector<double>& ang_vel){
  // get the properties of the particles and rigid body
  auto aosoa_velocity = Cabana::slice<2>     ( aosoa,    "velocity");
  auto aosoa_body_id = Cabana::slice<17>     ( aosoa,    "body_id");
  auto aosoa_dx0 = Cabana::slice<18>         ( aosoa,    "dx0");
...
// void compute_effective_force_and_torque_on_rigid_body(RBAoSoA rb, auto aosoa){
//   auto aosoa_position = Cabana::slice<0>     ( aosoa,    "aosoa_position");
//   auto aosoa_body_id = Cabana::slice<12>     ( aosoa,    "aosoa_body_id");
//   auto aosoa_frc_dem = Cabana::slice<14>     ( aosoa,    "aosoa_frc_dem");
...
void output_data(AoSoAType aosoa, int num_particles, int step, double time)
{
  ...
  Cabana::Experimental::HDF5ParticleOutput::
    writeTimeStep(
		  ...
		  ids, velocity, density, rad_s, body_id, mass);
}
				      ...
				      const int write_freq)
{
  ...
  */
  ...
  for ( std::size_t i = 0; i < no_of_bodies; ++i ) {
    ...
    for ( std::size_t j = 0; j < x_rb.size(); ++j )
      {
	x.push_back(x_rb[j] + i * body_length + i * 8. * body_spacing);
	y.push_back(y_rb[j]);
	z.push_back(z_rb[j]);
  ...
  */
  ...
  set_body_limits_of_rb(aosoa, rb_host);
  // setup the master rigid body properties
  setup_rigid_body_properties(aosoa, rb_host, rigid_limits);

  std::vector<double> linear_velocity_0 = {2., 2., 0.};
  std::vector<double> angular_velocity_0 = {0., 0., 3.};
  for ( std::size_t j = 0; j < rb.size(); ++j )
    {
      set_linear_velocity_rigid_body(aosoa, rb, j, linear_velocity_0);

      set_angular_velocity_rigid_body(aosoa, rb, j, angular_velocity_0);
    }
...
int main( int argc, char* argv[] )
{
  ...
  if ( argc < 7 )
	...
	<< "      write_freq      number of steps between output files\n";
      ...
      std::cerr << "\nfor example: ./01RBFreelyRotatingBodies 0.1 1.0 1.0 3 3.0 100\n";
      Kokkos::finalize();
      MPI_Finalize();
      return 0;
    }
